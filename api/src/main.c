...
main(int argc, char *argv[])
{
    ...
    */
...
#if __DEBUG__
    message("~D INADDR_ANY binded to APISOCK.\n");
#endif
    ...
    // SO_LINGER seçeneğini etkinleştir
// !    struct linger ling;
// !    ling.l_onoff = 1; // Beklemeyi etkinleştir
// !    ling.l_linger = 0; // Bekleme süresi (saniye cinsinden), sıfır olması soketi hemen kapatır
    ...
    if (DYN_buff == NULL) {
        message("~E Malloc failed for <<<<*DYN_buff>>>>\n");
...
// !    }
...
#if __DEBUG__
        message("~D Accept socket ACCEPTSOCK created successfully.\n");
#endif
        ...
        if (strncmp(DYN_buff, "GET / HTTP/1.1", 14) != 0) {
            if (strncmp(DYN_buff, "GET /break", 10) == 0) {
                message("~W Requested <<'/break'>> Breaking the law !\n");
                close(ACCEPTSOCK);
                break ;
            } else {
                close(ACCEPTSOCK);
